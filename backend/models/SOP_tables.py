...
class Response(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)  # Surrogate key for uniqueness
    incident_number = Column(String(50), ForeignKey(f"{DB_SCHEMA}.temp_incident.incident_number"), nullable=False)  # Foreign key to temp_incident
    ...
    temp_incident = relationship("TempIncident", back_populates="responses")  # Linked to TempIncident

    
class TempIncident(Base):
    __tablename__ = "temp_incident"
    __table_args__ = (
        {'schema': DB_SCHEMA}
    )

    incident_number = Column(String(50), primary_key=True)  # Primary key
    text_mme = Column(Text, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    # Relationship to Response
    responses = relationship("Response", back_populates="temp_incident")
    
    
class IncidentLog(VC_DB_Base):
    __tablename__ = "IncidentLog_TBL"
    __table_args__ = (
        {'schema': "dbo"}
    )

    # Columns you need for the query
    incidentlog_prk = Column(Integer, primary_key=True)  # Primary key column
    inlIncidentDetails_MEM = Column(Text)  # Column to be updated
